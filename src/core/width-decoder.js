 ...
 * @property {import('../types.js').DecodedBarcode} barcode
 * @property {number} moduleWidth Mean narrow-element width: the symbol's module size, in run
 *   units (pixels).
 */
 ...
 * @property {number} width Total width of the 9 elements.
 * @property {number} narrow Mean narrow-element width — the local module size.
 */

/**
 * @typedef {LineSymbol & { end: number }} SymbolMatch `end` is the index of the symbol's
 *   trailing quiet zone (which may lead into the next symbol).
 */
...
/**
 * @param {CharacterMatch} current
 * @param {CharacterMatch} previous
 */
...
export class Code39WidthDecoder {
  /** @type {import('../options.js').ResolvedDecodeOptions} */
  #options;

  /** @param {import('../options.js').DecodeOptions} [options] */
  constructor(options) {
   ...
   * @param {Runs} runs
   * @returns {import('../types.js').DecodedBarcode | null}
   */
  decode(runs) {
   ...
   * @param {Runs} runs
   * @returns {import('../types.js').DecodedBarcode[]}
   */
  decodeAll(runs) {
